...
**Cuarto matiz añadido el 7 de agosto de 2026**, tras un incidente real en la Fase 5.3: las herramientas de autofix (`ruff check --fix` en particular) no se lanzan nunca sin acotar su alcance a los archivos que ya se están revisando activamente. Un `--fix` sin acotar tocó 58 archivos de golpe en una sola pasada, mezclando cambios puramente mecánicos (saltos de línea, orden de imports) con la investigación real que estaba en curso en ese momento — el resultado fue imposible de revisar con confianza, y Alan lo frenó explícitamente. Regla adoptada desde entonces: **un commit por cada cambio de lint distinto** ("poco a poco"), y los hallazgos que no son puramente mecánicos (como `F401`, imports sin usar — donde borrar el import equivocado puede romper el registro de modelos de SQLAlchemy en silencio) se revisan uno por uno a mano antes de aplicarlos, nunca con `--fix` a ciegas sobre múltiples archivos a la vez.
...
**Pendiente resuelto** (sesión posterior de repaso completo del proyecto, antes de arrancar la Fase 5): `src/app/page.tsx` ya compone los tres componentes contra un `activeRequest` real, con `useAnalysisRequestSocket` abierto una única vez ahí arriba y pasado hacia abajo como props (`events`/`isConnected`) en vez de que `AgentTrace` y `ApprovalPanel` abrieran cada uno su propia conexión al mismo `analysis_request_id` — ver los comentarios "Fase 4.6 review" en el código.

### FASE 5 — Calidad, tests y observabilidad ✅ — 7 de agosto de 2026

Esta fase no traía sub-fases predefinidas en este documento — solo la línea genérica "testing en paralelo, mocking de LLMs, logging estructurado, CI". El desglose real en 5.1-5.4 se decidió junto con Alan al arrancar, con este orden explícito: CI y cobertura tienen más sentido una vez el propio código ya loguea de forma consistente, no antes.

**5.1 — Logging estructurado**: `app/logging_config.py` (nuevo) centraliza la configuración — `JSONFormatter` + `setup_logging(level)`, invocado una vez desde el lifespan de `main.py`, nivel ligado a `Settings.ENVIRONMENT`. Correlación por `analysis_request_id` vía `contextvars.ContextVar`, no vía `extra=` manual en cada llamada — se propaga sola porque el contexto se fija antes de lanzar las tareas async del fan-out de especialistas (`asyncio.gather`/`create_task` heredan una copia del contexto en el momento de creación).

**Gotcha real**, descubierto verificando a mano que `analysis_request_id` apareciera en el JSON de salida: el diseño inicial usaba un `logging.Filter` adjunto al logger raíz para inyectar el campo, y no aparecía. Causa raíz, confirmada con una reproducción mínima: **los `Filter` de un Logger solo se ejecutan para registros logueados directamente sobre ese logger — nunca se propagan a loggers hijos** (solo los `Handler` se propagan hacia arriba en la jerarquía). Se resolvió haciendo que `JSONFormatter.format()` lea `analysis_request_id_var.get()` directamente, sin ningún `Filter` de por medio.

**5.2 — CI (GitHub Actions)**: `.github/workflows/ci.yml`, dos jobs paralelos (`backend`/`frontend`) en vez de uno secuencial — el resumen de GitHub muestra cuál falló de un vistazo sin abrir logs. El job de backend levanta `postgres:16` y `redis/redis-stack-server` (no `redis` a secas — `langgraph-checkpoint-redis` necesita RediSearch) como *service containers*, en los mismos puertos que expone `docker-compose.yml` en local, así que la suite corre sin cambios.

**Hallazgo real al montarlo**: se añadió `alembic upgrade head` como paso independiente (smoke test, no parte de `pytest` — la suite nunca toca Alembic, construye el schema directo desde los modelos vía `Base.metadata.create_all()`). Al correrlo, reveló que la base de datos de desarrollo real llevaba **una migración de retraso** respecto a `head` sin que nadie lo hubiera notado — justo el tipo de deuda silenciosa que este paso existe para atrapar.

**5.3 — Ruff (lint + format)**: `pyproject.toml` nuevo — `select=["E","W","F","I","B"]`, `ignore=["B008"]` (falso positivo conocido de FastAPI con `Depends(...)` como valor por defecto), `per-file-ignores` para `E501` en tests, `extend-exclude=["alembic/versions"]` (no tiene sentido lintear migraciones autogeneradas).

**Incidente real de esta fase, con impacto directo en metodología**: al revisar si unos imports de modelos en `alembic/env.py`/`main.py` eran seguros de quitar, un `ruff check --fix` sin acotar tocó 58 archivos de golpe en una sola pasada. Alan lo frenó explícitamente y estableció una regla nueva, vigente desde entonces — ver el nuevo matiz añadido a la Adenda de metodología, sección "Convención de commits".

**5.4 — Cobertura de tests + componentes React + test de Alembic**: `[tool.coverage.report] fail_under = 95` en `pyproject.toml`, leído automáticamente por `pytest-cov` sin necesitar `--cov-fail-under` en el comando — verificado directamente forzando una cobertura baja antes de confiar en la afirmación. Se cerraron 5 huecos reales de cobertura en endpoints y en `runner.py`.

Componentes React pendientes desde la 5.1 (`AgentTrace`, `NewAnalysisForm`, `MetricsPanel`, `ApprovalPanel`) probados con `@testing-library/react` + `jsdom` (antes solo se testeaban funciones puras), mockeando `@/lib/api/client` con `vi.mock` — mismo principio que el backend: se parchea el módulo tal como lo importa el componente, no la implementación interna de `fetch`. `ApprovalPanel`, el más complejo (fuente en vivo por WebSocket vs. fallback por REST, envío de decisión, detección de que el grafo realmente se reanudó), recibió cobertura exhaustiva a petición explícita de Alan: los 4 estados del fallback, prioridad del evento en vivo sobre el REST, éxito/error al decidir, y las dos ramas válidas de "resumed" (`human_approval`/`post_comment`) frente a un nodo no relacionado.

Test de `alembic upgrade head` / `downgrade base` / `upgrade head` (round-trip) sobre `nexus_test`, la misma base que usa el resto de la suite. **Gotcha real, el más sutil de la fase**: el test pasaba en aislamiento pero fallaba al correr con la suite completa. Causa raíz: `alembic_version` (la tabla de control interno de Alembic) **no forma parte de `Base.metadata`**, así que `Base.metadata.drop_all()` nunca la toca. Como `tests/api/conftest.py` ya deja las tablas de la app borradas en su propio teardown, y `nexus_test` es una base persistente entre ejecuciones, Alembic se encontraba con `alembic_version` diciendo "ya estoy en head" mientras las tablas reales no existían — `upgrade("head")` se convertía en un no-op silencioso, y el `downgrade("base")` posterior fallaba contra tablas que nunca se habían recreado. Se corrigió borrando `alembic_version` explícitamente por SQL, además del `drop_all()` de los modelos.

**CI se rompió dos veces tras esta fase, por motivos ajenos al código**: primero con exit code 127 — `ruff` y `pytest-cov` se habían instalado en el entorno local durante la sesión pero nunca se añadieron a `requirements.txt`; corregido añadiendo las tres dependencias que faltaban (confirmado con un diff sistemático `pip freeze` vs. `requirements.txt`). Después, con el job de frontend fallando directamente con *"The job was not acquired by Runner of type hosted even after multiple attempts"* y un *"Internal server error"* a nivel de plataforma — infraestructura de GitHub Actions, no algo del repositorio; se resolvió solo, relanzando el workflow.

**Verificación**: `pytest -v --cov=app --cov-report=term-missing` (99% de cobertura, umbral 95%), `ruff check .` limpio, el round-trip de Alembic en aislamiento y dentro de la suite completa, `tsc --noEmit`, `eslint`, y `vitest run` (46 tests de frontend, desde los 7 iniciales de `agent-trace.test.ts`) — todo confirmado en verde por Alan tanto en local como en GitHub Actions tras relanzar el CI.
...
*Última actualización: 7 de agosto de 2026 — pendiente heredado de la Fase 4 resuelto (`src/app/page.tsx` ya compone `AgentTrace`/`ApprovalPanel`/`MetricsPanel` contra un `analysisRequestId` real); Fase 5 completa y verificada de extremo a extremo: 5.1 logging estructurado (JSON + `contextvars`), 5.2 CI en GitHub Actions (backend y frontend en paralelo, con `alembic upgrade head` como smoke test), 5.3 lint con ruff (con el incidente del `--fix` masivo que dio pie al nuevo matiz de la Adenda de metodología), 5.4 cobertura de tests al 99% (umbral 95%) + los cuatro componentes React pendientes desde la 5.1 + test de round-trip de Alembic. 178 tests de backend en verde (desde 142) y 46 de frontend (desde 0). Pendiente: Fase 6 (despliegue a producción), sin empezar.*
