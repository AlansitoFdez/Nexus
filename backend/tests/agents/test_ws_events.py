...
def test_router_node_chunk_becomes_specialists_started():
    ...
    events = build_event(chunk)

    assert events == [{"type": "specialists_started", "specialists": ["security", "performance"]}]
...
def test_two_specialists_finishing_in_the_same_superstep_both_produce_events():
    """Regression test: the Send() fan-out (Fase 2.4) can finish more
    than one specialist in the same astream() chunk — a chunk isn't
    guaranteed to carry only one {node_name: delta} pair. Dropping every
    key but the first would leave whichever specialist finished second
    stuck showing "running" in the dashboard forever, since nothing
    would ever tell the frontend it was done."""
    chunk = {
        "security_agent": {"findings": [{"severity": "high"}], "node_history": ["security_agent"]},
        "performance_agent": {"findings": [], "node_history": ["performance_agent"]},
    }

    events = build_event(chunk)

    assert len(events) == 2
    assert {"type": "specialist_finished", "specialist": "security", "findings_count": 1, "failed": False} in events
    assert {"type": "specialist_finished", "specialist": "performance", "findings_count": 0, "failed": False} in events
...
def test_entry_node_chunk_becomes_node_finished():
    ...
    assert events == [{"type": "node_finished", "node": "entry"}]
...
def test_empty_interrupt_tuple_returns_empty_list():
    assert build_event({"__interrupt__": ()}) == []


def test_unrecognized_node_returns_empty_list():
    assert build_event({"some_future_node": {"node_history": ["some_future_node"]}}) == []
